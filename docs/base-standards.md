...
- **Type Safety**: All code must be fully typed. This is a TypeScript-only project — no `any`, no implicit types.
...
- **English Only**: All technical artifacts must always use English, including:
    - Code (variables, functions, types, comments, error messages, log messages)
    - Documentation (README, guides, data model)
...
- [Frontend Standards](./frontend-standards.md) — React 19 component patterns, state management via useReducer + Context, design-token CSS, and Vite configuration
